...
class Akvarium:
    ...
    def leggTilFisk(self,fisk):
        if fisk.lengde >= 0.3*self.l:
            print(f"For kort akvarium for {fisk.navn}. Prøv med en lengde på minimum {fisk.lengde*3} cm")
        elif fisk.lengde >= 0.3*self.b:
            print(f"For smalt akvarium for {fisk.navn}. Prøv med en bredde på minimum {fisk.lengde*3} cm")
        elif fisk.lengde >= 0.3*self.h:
            print(f"For lavt akvarium for {fisk.navn}. Prøv med en høyde på minimum {fisk.lengde*3} cm")
        else:
            self.fisker.append(fisk)
            print(f"{fisk.navn} lagt oppi akvariet. Nå er det {len(self.fisker)} oppi akvariet.")
...
class Fisk:
    ...
    def spis(self):
        return f"{self.navn} liker å spise" # Endret til retur så den kan brukes av underklasser.
